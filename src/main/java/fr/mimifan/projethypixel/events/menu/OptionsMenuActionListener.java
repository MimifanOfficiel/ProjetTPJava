...
/**
 * Class FileMenuActionListener
 * Performs actions when clicking on the Options Menu.
 */
        ...
        switch (e.getActionCommand()){
            case "setplayer" -> {
                ...
                if (result == JOptionPane.OK_OPTION) {
                    try {
                        for (int i = 0; i < MainFrame.getInstance().getPlayersPane().getTabCount(); i++) {
                            if(MainFrame.getInstance().getPlayersPane().getTitleAt(i).equals(valueField.getText())) return;
                        }
